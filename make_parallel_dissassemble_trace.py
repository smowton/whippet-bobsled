...
import trace_generators.parallel_trace_dissassemble_generator as gen
import serialize_trace.serializer as ser

if len(sys.argv) < 3:
    print("Usage: make_parallel_dissassemble_trace.py some.mdl some.nbt [-f] [--lockstep] [--sort-tiers]", file = sys.stderr)
    sys.exit(1)
...
trace = gen.build_trace(model, lockstep, sort_tiers)
